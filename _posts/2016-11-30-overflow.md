...
If you happen to be using [jq] to provide tweet identifier datasets to hydrate
for others to hydrate you may be interested in this significant difference
between `jq .id` and `jq .id_str`.
...
string with this instead, but I wouldn't have found my error that way. So when
my grep on the `.id_str` failed I couldn't figure out why my tweet wasn't found.
I tried to get just the one tweet using grep and it wasn't in there either!
...
I don't know that jq was in GWU's pipeline but I know it has been in mine for a
few months for creating the tweet id datasets. I had been using `.id_str` until
fairly recently when I saw `id` being used. Luckily there were just a handful of
